...
class IP_Header:
    ...
    def retrieve_ihl(self):
        return self.ip_header_len
 
    def retrieve_total_len(self):
...
class TCP_Header:
    ...
    def tcp_get_info(self):
        return (self.src_port, self.dst_port, self.seq_num,
                self.ack_num, self.data_offset, self.flags, self.window_size)
    ...
    def get_flags(self,buffer):
        ...
        #print(f"FIN {fin}, SYN {syn}, RST {rst}, ACK {ack}")
...
class packet():
    ...
    def packet_set_headers(self, pack):
        #self.timestamp_set(pack[0], pack[1],orig_time)
...
class global_header():
    ...
    def set_headers(self, pack, endian):
        self.magic_number = pack[0]
        self.version_major = pack[1]
        self.version_minor = pack[2]
        self.thiszone = pack[3]
        self.sigfigs = pack[4]
        self.snaplen = pack[4]
        self.network = pack[5]
